...
BEGIN
	...
	START TRANSACTION; # début de la transaction de test
    ...
    ROLLBACK; # remise à zéro des données de test
END $$
DELIMITER ;
...
BEGIN
    ...
    ROLLBACK; # remise à zéro des enregistrements et des modifications de test
END $$
DELIMITER ;
...
BEGIN
	...
	SET _pv_finaux_aventuriers = (SELECT sum(Aventurier.point_vie) FROM Aventurier 
		INNER JOIN Expedition_aventurier ON Expedition_aventurier.aventurier = Aventurier.id_aventurier
        WHERE expedition = 6000);
