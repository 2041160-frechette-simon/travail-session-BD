...
BEGIN
	...
	IF(_id_mage IS NOT NULL AND _id_humanoide IS NOT NULL AND _moment_visite IS NOT NULL)
    THEN 
 ...
 *@param _id_salle IN
 *@param _id_expedition IN
 *@dependencies: verifier_vitalite_aventurier_expedition, verifier_vitalite_monstre_salle, Malediction_affaiblissement
...
BEGIN
	...
	WHILE verifier_vitalite_aventurier_expedition(_id_expedition) = 1 AND verifier_vitalite_monstre_salle(_id_salle, _moment_visite) = 1
    DO
		...
		SET _monstres_en_vie = (SELECT count(id_monstre) FROM Salle 
        INNER JOIN Affectation_salle ON salle = id_salle
        INNER JOIN Monstre ON Affectation_salle.monstre = Monstre.id_monstre
        WHERE  Salle.id_salle = _id_salle AND point_vie > 0 AND _moment_visite BETWEEN Affectation_salle.debut_affectation AND Affectation_salle.fin_affectation);
		...
		IF _monstres_en_vie > 0 THEN 
			...
			SET _aventuriers_en_vie = (SELECT count(Aventurier.id_aventurier) FROM Aventurier 
			...
			WHERE point_vie > 0 AND Expedition.id_expedition = _id_expedition);
...
BEGIN
	...
	ELSE
        ...
        IF(verifier_vitalite_aventurier_expedition(_id_expedition) = 1)
        THEN
			CALL Piller_salle(_id_salle, _id_expedition);
        END IF;
    END IF;
...
BEGIN
    ...
    DECLARE _nom_famille_existante VARCHAR(255);
    ...
    SET _nom_famille_existante = (SELECT Famille_monstre.nom_famille FROM Famille_monstre WHERE Famille_monstre.nom_famille = _nom_famille);
    
    IF _nom_famille_existante IS NULL THEN
		SIGNAL SQLSTATE '02001'
    ...
    SET _point_vie = (SELECT point_vie_maximal FROM Famille_monstre WHERE id_famille = _id_famille);
    SET _attaque = (SELECT degat_base FROM Famille_monstre WHERE id_famille = _id_famille);
    
    START TRANSACTION;
    INSERT INTO Monstre(nom, code_employe, point_vie, attaque, numero_ass_maladie, id_famille, experience)
...
BEGIN
    ...
    INSERT INTO Famille_monstre(nom_famille, degat_base, point_vie_maximal)
